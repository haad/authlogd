...
char buf2[] = "<35>1 2003-10-11T22:14:15.003Z mymachine.example.com evntslog - ID47 [exampleSDID@0 iut=\"3\" eventSource=\"Application\" eventID=\"1011\"] BOMAn application event log entry";


ATF_TC(authlog_msg_tc);
ATF_TC_HEAD(authlog_msg_tc, tc)
{
	atf_tc_set_md_var(tc, "descr", "Tries to log message through auth socket.");
}

ATF_TC_BODY(authlog_msg_tc, tc)
{
	int s;
	...
	if ((s = socket(AF_UNIX, SOCK_STREAM, 0)) == -1)
			atf_tc_fail("socket call failed");
	
	/*
	 * Connect to the server
	 */
	if ((connect(s, (const struct sockaddr *)&sa, sizeof(struct sockaddr_un))) == - 1)
		atf_tc_fail("connect to authlogd socket failed");
...
ATF_TP_ADD_TCS(tp)
{
	ATF_TP_ADD_TC(tp, authlog_msg_tc);
}
